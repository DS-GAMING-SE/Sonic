...
{
    public class OldBoost : GenericCharacterMain
    {
        ...
        protected virtual void PlayBoostEffects()
        {
            if (boosting && boostEffectCooldown <= 0)
            {
                ...
                if (powerBoosting)
                {
                    Util.PlaySound(boostChangedEffect ? GetSoundString() : OldBoost.boostChangeSoundString, base.gameObject);
                    if (base.isAuthority)
                    {
        ...
        {
            if (typeof(OldBoost).IsAssignableFrom(skill.activationState.stateType))
            {
